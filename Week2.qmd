...
![](images/Week2-FalsePositives.png){fig-align="center" width="60%"}
...
![](images/Week2-Code.png){fig-align="center" width="75%"}
::::
::::::::
...
\begin{aligned}
\operatorname{FWER} 
&= P_{H_0}(\text{at least one of the $m$ null hypotheses is rejected}) \\
&= P_{H_0}\left(
\bigcup_{j=1}^{m}
\left\{p_j \leq \frac{\alpha}{m}\right\}
\right) \\
&\leq \sum_{j=1}^{m}
P_{H_0}\left(
p_j \leq \frac{\alpha}{m}
\right) \\
&= m\left(\frac{\alpha}{m}\right) \\
&= \alpha.
\end{aligned}
$$
:::
...
1. Calculate the observed test statistics:
   $$
   T_1,\ldots,T_m.
   $$

2. Permute the data under $H_0$ and recompute all $m$ test statistics.

3. For permutation $b$, keep only the most extreme statistic:
   $$
   T_{\max}^{(b)}=\max_j |T_j^{(b)}|.
   $$

4. Repeat for $b=1,\ldots,B$ to form the null distribution of $T_{\max}$.

5. For $\alpha=0.05$, use the 95th percentile as the cutoff $c$.
...
# we want to test the hypothesis: H_0: correlation_j=0
observed_max <- max(abs(cor(X, y))) # store max correlation
permuted_max <- numeric(1000)

for (b in seq_len(1000)) { # repeat 1000 times
  y_shuffled <- sample(y) # shuffle y
  permuted_max[b] <- max(abs(cor(X, y_shuffled))) # recalculate correlations, take max one
}

cutoff <- quantile(permuted_max, 0.95)
cutoff
